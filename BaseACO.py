...
def ACO(NB_ITERATIONS,NB_FOURMIS,NB_VILLES = 250):
    ...
    meilleure_distance_globale = np.inf
    ...
    for iteration in range(NB_ITERATIONS):
        ...
        for f in range(NB_FOURMIS):
            ...
            while len(chemin) < NB_VILLES:
                ...
                choix_ville = random.choices(villes_accessibles, weights=probabilites)[0]
...
with open("meilleur-chemin-nbr-fourmis.csv","w",newline="", encoding="utf-8") as fichier:
    ...
    for i in range(1,81,2):
        resultat = ACO(50,i)
        writer.writerow([i,resultat[1],resultat[2]])
